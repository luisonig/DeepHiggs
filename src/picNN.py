...
def train_pixels(ip, x_train, y_train, x_devel, y_devel):
    ...
    x_devel=x_devel.reshape(x_devel.shape[0],1,x_devel.shape[1],x_devel.shape[2])
    

    y_train = np_utils.to_categorical(y_train, 2)
    y_devel = np_utils.to_categorical(y_devel, 2)
    ...
    model.compile(loss='categorical_crossentropy',
              optimizer='adam',
              metrics=['accuracy'])
    model.fit(x_train, y_train,batch_size=32, nb_epoch=ip.max_steps, verbose=1)	
